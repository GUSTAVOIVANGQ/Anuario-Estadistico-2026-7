...
3. Construir la tabla final usada en la gráfica.
4. Guardarla con `context.write_data_used(...)`. El método la guarda e imprime.
5. Registrar cada fórmula con `context.record_calculation(...)`.
6. Renderizar el párrafo con `context.render_text(...)` cuando corresponda.
7. Generar el PNG conservando el diseño del anuario 2024.
8. Generar `slide_path` cuando la página del referente combine narrativa y
   gráfica en una composición específica. Esto permite replicar cada página sin
   imponer un diseño genérico.
9. Devolver `{"figure_path": ruta}` y, si existe, `slide_path`.
