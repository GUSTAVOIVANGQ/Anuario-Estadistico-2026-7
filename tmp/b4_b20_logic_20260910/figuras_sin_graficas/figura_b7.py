...
DATA_PATH = ensure_bit_table("TD_PENETRACIONES_TELFIJA_ITE_VA.csv", "B.7")

df_all = read_csv_flexible(DATA_PATH)
df_all["ANIO"] = numeric(df_all["ANIO"])
df_all["MES"] = numeric(df_all["MES"])
df_all["P_NRES_H_TELFIJA_E"] = numeric(df_all["P_NRES_H_TELFIJA_E"])
...
if "Nacional" not in data:
    raise RuntimeError("BIT no contiene la fila Nacional para B.7; no se estimará a partir de promedios estatales.")
nacional_val = float(data["Nacional"])
prev = df_all[(df_all["ANIO"] == ANIO_PREVIO) & (df_all["MES"] == 12) & (df_all["ENTIDAD"].astype(str).str.strip().str.casefold() == "nacional")]["P_NRES_H_TELFIJA_E"]
tasa_crecimiento = ((nacional_val / float(prev.iloc[0])) - 1) * 100 if len(prev) and float(prev.iloc[0]) != 0 else float("nan")
save_audit("B.7", df[["ANIO", "MES", "ENTIDAD", "P_NRES_H_TELFIJA_E"]])
...
print(f"Figura B.7. Líneas del Servicio Fijo de Telefonía No Residencial por cada 100 unidades "
...
print("\nValores por entidad federativa (líneas no residenciales por cada 100 unidades económicas):")
tabla = pd.DataFrame({
    "ENTIDAD": list(data.keys()),
    "P_NRES_H_TELFIJA_E": list(data.values()),
})
tabla["RANGO"] = tabla["P_NRES_H_TELFIJA_E"].apply(get_label)
tabla["COLOR"] = tabla["P_NRES_H_TELFIJA_E"].apply(get_color)
print(tabla.sort_values("P_NRES_H_TELFIJA_E", ascending=False).to_string(index=False))
