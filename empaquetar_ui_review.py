"""
empaquetar_ui_review.py
=======================
Genera un archivo ZIP del proyecto **sin** datos descargables (data/raw/**),
listo para que otro programador revise y mejore la UI de las gráficas.

El ZIP incluye:
  - Todo el código fuente (src/, scripts/, web/src, config/, tests/, docs/)
  - Assets (fonts, imágenes de referencia visual)
  - Archivos de configuración (pyproject.toml, requirements.txt, etc.)
  - Un script demo autónomo ``demo_graficas_ui.py`` que genera ~10 gráficas
    representativas con datos SINTÉTICOS para ver estilos sin datos reales.

Uso:
    python empaquetar_ui_review.py
    # Genera: anuario_2026_ui_review_<YYYYMMDD>.zip en la raíz del proyecto
"""
...
OUTPUT_ZIP = ROOT / f"anuario_2026_ui_review_{TODAY}.zip"

# Directorios y archivos a EXCLUIR del ZIP
EXCLUDE_DIRS = {
    ".venv",
    ".git",
    "__pycache__",
    ...
    ".pptx-build",
    "node_modules",
    "build",         # resultados generados, no código
    "entrega",       # PDFs/PPTX generados
    "reportes",      # reportes de corridas
    "output",        # salidas temporales
    "tmp",
    "web/dist",      # frontend compilado
    # ← el directorio de datos descargables se excluye por patrón:
    # data/raw/** (sólo se conserva data/manual/*)
}

EXCLUDE_EXTENSIONS = {".pyc", ".pyo", ".part"}

# Archivos grandes a excluir de la raíz
EXCLUDE_ROOT_FILES = {
    "anuarioestadistico2024vf_0.pdf",  # PDF de referencia (45 MB)
    "MANIFEST_SHA256.txt",             # manifiesto de hashes (397 KB)
    OUTPUT_ZIP.name,                   # el propio ZIP que estamos creando
}

# Archivos de screenshots y referencias de imagen grandes
EXCLUDE_ASSET_SUBDIRS = {"screenshots"}  # assets/screenshots (no es código)


# ---------------------------------------------------------------------------
# Script demo autónomo (se embebe en el ZIP como demo_graficas_ui.py)
# ---------------------------------------------------------------------------

DEMO_SCRIPT = r'''#!/usr/bin/env python3
"""
demo_graficas_ui.py
===================
...
def main() -> None:
    ...
    print()
    print(f"[OK] {len(list(OUT_DIR.glob('*.png')))} imagenes generadas en {OUT_DIR.resolve()}")
    print()
    print("Para ver las imagenes en Windows:")
...
# ---------------------------------------------------------------------------
# README rápido para el revisor
# ---------------------------------------------------------------------------

README_REVIEW = """# Anuario Estadístico 2026 — Paquete UI Review

Este paquete contiene **todo el código** del proyecto sin los datos
descargables (`data/raw/`), listo para revisar y mejorar la UI de las
gráficas.
...
### 2. Ver las gráficas de muestra (sin datos reales)
...
Este script genera **10 gráficas representativas** con datos sintéticos
que reproducen fielmente el estilo visual del proyecto (paleta, tipografía,
marcadores de título, pies de fuente, etc.).

### 3. Ejecutar el proyecto completo (requiere datos)

Los datos reales se descargan con los scripts en `scripts/downloaders/`
o siguiendo las instrucciones del `README.md` principal.
...
## Estructura del código relevante para UI

| Ruta | Descripción |
|------|-------------|
| `src/anuario2026/ui_2024.py` | Módulo central de UI: paleta, helpers, parche de crédito |
| `scripts/figures/figura_*.py` | Un script por figura (A.1–G.1) |
| `web/src/App.jsx` | Frontend React de la interfaz web |
| `web/src/styles.css` | Estilos CSS de la interfaz web |
| `assets/fonts/Noto_Sans/` | Tipografía institucional |
| `demo_graficas_ui.py` | Demo autónomo con datos sintéticos |

## Paleta institucional

| Rol | Color |
|-----|-------|
| Teal oscuro (series principales) | `#132b2d` |
| Teal medio | `#335a5c` |
| Teal claro | `#86adae` |
| Acento / marcador de título | `#4a7d75` |
| Fondo de axes | `#F8F8FA` |
| Texto | `#3c3c3b` |

Ver `demo_output/demo_paleta_institucional.png` para el panel completo.
"""
...
def _should_exclude_dir(rel_parts: tuple[str, ...]) -> bool:
    """True si algún componente del path relativo pertenece a un dir excluido."""
    for part in rel_parts:
        if part in EXCLUDE_DIRS:
            return True
    # data/raw específicamente
    if len(rel_parts) >= 2 and rel_parts[0] == "data" and rel_parts[1] == "raw":
        return True
    # assets/screenshots
    if len(rel_parts) >= 2 and rel_parts[0] == "assets" and rel_parts[1] in EXCLUDE_ASSET_SUBDIRS:
        return True
    return False


def _should_exclude_file(path: Path, rel: Path) -> bool:
    ...
    # Excluir archivos raíz grandes
    if len(parts) == 1 and parts[0] in EXCLUDE_ROOT_FILES:
        return True

    # Excluir el propio script de empaquetado
    if len(parts) == 1 and parts[0] == Path(__file__).name:
        return True

    # Excluir el ZIP de salida si ya existe
    if len(parts) == 1 and parts[0] == OUTPUT_ZIP.name:
        return True

    # Excluir archivos dentro de dirs excluidos
    if _should_exclude_dir(parts[:-1]):
        return True
...
def build_zip() -> None:
    ...
    with zipfile.ZipFile(OUTPUT_ZIP, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        # 1. Recorrer todos los archivos del proyecto
        for path in sorted(ROOT.rglob("*")):
            ...
            if _should_exclude_file(path, rel):
                excluded += 1
                continue
            arc_name = f"anuario_2026_ui_review/{rel.as_posix()}"
            zf.write(path, arcname=arc_name)
            included += 1
            total_bytes += path.stat().st_size

        # 2. Agregar el script demo autónomo
        zf.writestr(
            "anuario_2026_ui_review/demo_graficas_ui.py",
            DEMO_SCRIPT.lstrip("\n"),
        )
        included += 1

        # 3. Agregar el README de revisión
        zf.writestr(
            "anuario_2026_ui_review/README_UI_REVIEW.md",
            README_REVIEW.lstrip("\n"),
        )
        included += 1

    zip_size = OUTPUT_ZIP.stat().st_size

    print(f"[OK] Archivos incluidos : {included:,}")
    print(f"     Archivos excluidos : {excluded:,}  (data/raw, .venv, build, etc.)")
    print(f"     Tamano del contenido: {total_bytes / 1_048_576:.1f} MB")
    print(f"     Tamano del ZIP      : {zip_size / 1_048_576:.1f} MB")
    print()
    print(f"     -> {OUTPUT_ZIP}")
    print()
    print("El ZIP incluye demo_graficas_ui.py -- ejecutalo con:")
    print("    pip install matplotlib numpy pandas pillow")
    print("    python demo_graficas_ui.py")
