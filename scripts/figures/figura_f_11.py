...
TEXT="#3c3c3b"; BLUE="#335a5c"; SALMON="#86adae"; BACKGROUND="#F8F8FA"
...
def _plot(data,out,root):
    plt.rcParams.update({"font.family":_font(root)});fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.055),.95,.87,boxstyle="round,pad=.012,rounding_size=.02",fc=BACKGROUND,ec="none",transform=fig.transFigure,zorder=-2));fig.text(.047,.887,"   ",fontsize=2,va="center",bbox=dict(boxstyle="round,pad=1.6,rounding_size=.2",fc="#4a7d75",ec="none"));fig.text(.064,.887,"Figura F.11.",color=TEXT,fontsize=16,fontweight="bold",va="center");fig.text(.171,.887,"Personas con mayor riesgo de violencia en Internet, por sexo (2023)",color=TEXT,fontsize=16,va="center")
    ax=fig.add_axes([.23,.16,.70,.65]);ordered=data.iloc[::-1].reset_index(drop=True);y=np.arange(len(ordered));h=.34;b1=ax.barh(y-h/2,ordered.hombres_pct,height=h,color=BLUE,label="Hombres");b2=ax.barh(y+h/2,ordered.mujeres_pct,height=h,color=SALMON,label="Mujeres");ax.set_yticks(y,ordered.categoria);ax.set_xlim(0,65);ax.set_xticks(np.arange(0,66,10),[f"{x}%" for x in range(0,66,10)]);ax.tick_params(axis="both",length=0,labelsize=10,colors=TEXT);ax.grid(axis="x",color="#d1d1d1",lw=.8);ax.set_axisbelow(True);ax.spines[:].set_visible(False);ax.set_facecolor(BACKGROUND);ax.legend(loc="lower right",frameon=False,ncol=2,labelcolor=TEXT,fontsize=11)
    for bars in (b1,b2):
        for bar in bars: ax.text(bar.get_width()+.5,bar.get_y()+bar.get_height()/2,f"{bar.get_width():.1f}%",va="center",fontsize=9,fontweight="bold",color=TEXT,bbox=dict(boxstyle="round,pad=.22,rounding_size=.8",fc="white",ec=bar.get_facecolor(),lw=.8))
