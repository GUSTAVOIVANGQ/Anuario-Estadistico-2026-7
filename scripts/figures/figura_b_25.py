...
import pandas as pd
FIGURE_ID="B.25"; SOURCE_ID="crt_bit_todo_2025_q2"; TABLE="TD_IHH_TVRES_ITE_VA.csv"; TEXT="#4B4B83"; TEAL="#ADDCDF"; CREAM="#FBFBF7"
def _member(z):
    for n in z.namelist():
...
def _plot(d,m,out,root):
    ...
    fig.text(.045,.9," ",bbox=dict(boxstyle="round,pad=1.5",fc="#F58F82",ec="none"),zorder=20); fig.text(.061,.9,"Figura B.25.",fontsize=14,fontweight="bold",color=TEXT,va="center",zorder=21); fig.text(.151,.9,f"Índice de concentración del Servicio de Televisión Restringida (2015-{m['anio']})",fontsize=14,color=TEXT,va="center",zorder=21)
