...
def _plot(data,out,root):
    ...
    bm=ax.bar(x-width/2,data.mujeres_pct,width=width,label="Mujeres",color=women,edgecolor="none",zorder=2); bh=ax.bar(x+width/2,data.hombres_pct,width=width,label="Hombres",color=men,edgecolor="none",zorder=2)
    ...
    fig.add_artist(patches.Rectangle((.060,.916),.009,.020,transform=fig.transFigure,facecolor="#4a7d75",edgecolor="none")); fig.text(.075,.926,"Figura F.13.",fontsize=14,fontweight="bold",color=text,va="center"); fig.text(.168,.926,"Personas con mayor riesgo de violencia mediante telefonía móvil, por sexo (2023)",fontsize=14,color=text,va="center"); fig.legend(loc="lower center",bbox_to_anchor=(.5,.11),ncol=2,frameon=False,fontsize=10)
