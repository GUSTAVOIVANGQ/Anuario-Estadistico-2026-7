...
import numpy as np
import pandas as pd
FIGURE_ID="F.13";SOURCE_ID="ift_tercera_encuesta_usuarios_2023_base";PERIOD="2023";TEXT="#4B4B83";BLUE="#317DA3";SALMON="#F58F82";BACKGROUND="#FBFBF7"
...
def _plot(d,out,root):
    plt.rcParams.update({"font.family":_font(root)});fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.055),.95,.87,boxstyle="round,pad=.012,rounding_size=.02",fc=BACKGROUND,ec="none",transform=fig.transFigure,zorder=-2));fig.text(.047,.887,"•",color=SALMON,fontsize=20,va="center");fig.text(.064,.887,"Figura F.13.",color=TEXT,fontsize=16,fontweight="bold",va="center");fig.text(.171,.887,"Personas con mayor riesgo de violencia mediante telefonía móvil, por sexo (2023)",color=TEXT,fontsize=16,va="center")
    ax=fig.add_axes([.25,.16,.68,.65]);ordered=d.iloc[::-1].reset_index(drop=True);y=np.arange(len(ordered));h=.34;b1=ax.barh(y-h/2,ordered.hombres_pct,height=h,color=BLUE,label="Hombres");b2=ax.barh(y+h/2,ordered.mujeres_pct,height=h,color=SALMON,label="Mujeres");ax.set_yticks(y,ordered.categoria);ax.set_xlim(0,55);ax.set_xticks(range(0,56,10),[f"{x}%" for x in range(0,56,10)]);ax.tick_params(axis="both",length=0,labelsize=10,colors=TEXT);ax.grid(axis="x",color="#DCEBE9",lw=.8);ax.set_axisbelow(True);ax.spines[:].set_visible(False);ax.set_facecolor(BACKGROUND);ax.legend(loc="lower right",frameon=False,ncol=2,labelcolor=TEXT,fontsize=11)
    for bars in (b1,b2):
        for b in bars:ax.text(b.get_width()+.45,b.get_y()+b.get_height()/2,f"{b.get_width():.1f}%",va="center",fontsize=9,fontweight="bold",color=TEXT)
