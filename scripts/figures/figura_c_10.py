...
import pandas as pd
FIGURE_ID="C.10";SOURCE_ID="crt_bit_todo_2025_q2";TABLE="TD_IHH_TELMOVIL_ITE_VA.csv";TEXT="#3c3c3b"
def load_raw(path):
...
def _plot(d,m,out):
    fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.045),.95,.89,boxstyle="round,pad=.01,rounding_size=.018",lw=0,fc="#F8F8FA",transform=fig.transFigure,zorder=-1));fig.text(.045,.9," ",bbox=dict(boxstyle="round,pad=1.5",fc="#4a7d75",ec="none"),zorder=20);fig.text(.061,.9,"Figura C.10.",fontsize=14,fontweight="bold",color=TEXT,va="center",zorder=21);fig.text(.153,.9,f"Índice de concentración del servicio móvil de telefonía (2013-{m['anio']})",fontsize=14,color=TEXT,va="center",zorder=21)
    ax=fig.add_axes([.11,.17,.80,.68]);rev=d.iloc[::-1].reset_index(drop=True);bars=ax.barh(range(len(rev)),rev.ihh,color="#335a5c",height=.58);ax.set_yticks(range(len(rev)),rev.anio.astype(str),color=TEXT);ax.set_xticks([]);ax.spines[:].set_visible(False);xmax=rev.ihh.max()
