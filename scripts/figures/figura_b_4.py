"""B.4: script autónomo de adquisición, cálculo, reporte, texto y gráfica."""
...
def _base(figure_id: str, year: int, start: int | None = None) -> tuple[plt.Figure, plt.Axes]:
    ...
    fig.add_artist(patches.FancyBboxPatch(
        (0.045, 0.105), 0.91, 0.805, transform=fig.transFigure,
        boxstyle="round,pad=0.008,rounding_size=0.018",
        facecolor=CREAM, edgecolor="#EEF0EE", linewidth=0.9, zorder=-5,
    ))
    fig.add_artist(patches.FancyBboxPatch(
        (0.066, 0.205), 0.868, 0.615, transform=fig.transFigure,
        boxstyle="round,pad=0.008,rounding_size=0.018",
        facecolor="white", edgecolor="#F0F0EF", linewidth=0.8, zorder=-3,
    ))
    ax = fig.add_axes([0.105, 0.275, 0.795, 0.49])
    fig.add_artist(patches.FancyBboxPatch((0.058, 0.864), 0.007, 0.018, transform=fig.transFigure,
                   boxstyle="round,pad=0,rounding_size=0.002", facecolor=SALMON, edgecolor="none"))
    fig.text(0.072, 0.873, f"Figura {figure_id}.", fontsize=13.2, fontweight="bold", color=TEXT, va="center")
    period = f" ({start}-{year})" if start else ""
    fig.text(0.148 if len(figure_id) == 3 else 0.156, 0.873, TITLES[figure_id] + period,
...
def _footer(fig: plt.Figure, source: str, note: str = "") -> None:
    ...
    if note:
        fig.text(0.058, 0.120, "Nota:", fontsize=8, fontweight="bold", color=TEXT, va="top")
        fig.text(0.089, 0.120, textwrap.fill(note, 210), fontsize=8, color=TEXT, va="top")
...
def _plot_series(figure_id: str, data: pd.DataFrame, meta: dict, output: Path) -> None:
    ...
    baseline = 10_000_000
    ceiling = max(34_000_000, math.ceil(y.max() / 2_000_000) * 2_000_000 + 2_000_000)
    ax.set_ylim(baseline, ceiling)
    ax.set_xlim(-0.25, len(data) - 0.75)
    ax.fill_between(x, baseline, y, color=LIGHT, alpha=0.20)
    ax.vlines(x, baseline, y, color=TEAL, linewidth=0.70, alpha=0.80)
    ax.plot(x, y, color=TEAL, linewidth=1.05, marker="o", markersize=3.0, label=legend)
    step = 1 if len(data) <= 30 else 2
    ax.set_xticks(x[::step], data["ANIO"].astype(str).iloc[::step], rotation=90, fontsize=7.5, fontweight="bold")
    ax.set_yticks(np.arange(baseline, ceiling + 1, 2_000_000))
    ax.grid(axis="y", color=GRID, linewidth=0.55, alpha=0.52)
    ...
    ax.add_patch(patches.FancyBboxPatch(
        (0.004, -0.088), 0.992, 0.088, transform=ax.transAxes,
        boxstyle="round,pad=0.003,rounding_size=0.014",
        facecolor="white", edgecolor=TEXT, linewidth=0.75,
        clip_on=False, zorder=-1,
    ))
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, 1.035), frameon=False,
    ...
    ax.annotate(f"{y[0]:,.0f}", (x[0], y[0]), xytext=(-10, 18), textcoords="offset points",
                ha="left", fontsize=8.2, color=TEXT)
    note = ""
    if figure_id == "B.8":
