"""B.19: script autónomo de adquisición, cálculo, reporte, texto y gráfica."""
...
def _plot_series(figure_id: str, data: pd.DataFrame, meta: dict, output: Path) -> None:
    ...
    if figure_id == "B.19":
        label_y = y[-1] + max(y) * 0.12
        ax.set_ylim(top=max(ax.get_ylim()[1], max(y) * 1.20))
        ax.annotate(
            label_value, xy=(x[-1], y[-1]), xytext=(x[-1], label_y),
            textcoords="data", ha="center", va="bottom", fontsize=8.5,
            fontweight="bold", color=TEXT,
            bbox=dict(boxstyle="round,pad=0.25", facecolor="white", edgecolor="none"),
            arrowprops=dict(arrowstyle="-", color=TEAL, linewidth=0.9,
