"""Figura D.9: seguridad percibida en compras por Internet (ECSI 2024)."""
...
FIGURE_ID, SOURCE_ID, PERIOD = "D.9", "ift_ecsi_2024_base", "2024"
...
REFERENCE = [[3.4,52.7,14.1,17.6,11.9],[3.9,52.1,11.0,16.7,15.5],[3.8,49.6,7.5,18.3,19.8],[4.5,52.7,7.4,18.2,16.8],[2.9,43.2,8.0,15.7,29.1]]


def load_and_calculate(path: Path) -> pd.DataFrame:
    data=pd.read_csv(path,usecols=["rescate_internet","edad_gpos","seg_comp","fac_per"],low_memory=False)
    data["fac_per"]=pd.to_numeric(data.fac_per,errors="coerce").fillna(0); data=data.loc[data.rescate_internet.eq(1)].copy(); data["seg_comp"]=data.seg_comp.fillna(9)
    rows=[]
    for age_code,age in AGES:
        ...
        for level_code,level in LEVELS:
            numerator=float(sub.loc[sub.seg_comp.eq(level_code),"fac_per"].sum())
            rows.append({"edad":age,"codigo_edad":age_code,"nivel":level,"codigo_nivel":level_code,"porcentaje":numerator/denominator*100,"numerador_ponderado":numerator,"denominador_ponderado":denominator})
...
def _plot(data:pd.DataFrame,output:Path,root:Path)->None:
    ...
    fig.text(.055,.88,"   ",fontsize=2,va="center",bbox=dict(boxstyle="round,pad=1.6,rounding_size=.2",fc="#4a7d75",ec="none")); fig.text(.073,.88,"Figura D.9.",color=TEXT,fontsize=16,fontweight="bold",va="center")
    fig.text(.18,.88,"Seguridad percibida al realizar compras por Internet, por grupo de edad (2024)",color=TEXT,fontsize=16,va="center")
    ...
    ax.set_ylim(0,66); ax.set_yticks(range(0,61,10),[f"{x}%" for x in range(0,61,10)]); ax.set_xticks(x,[x[1] for x in AGES],color=TEXT)
...
def generate(context):
    print("  D.9 | Descarga o reutilización de la base oficial ECSI 2024")
