...
import numpy as np
import pandas as pd
FIGURE_ID="C.16";SOURCE_ID="crt_bit_todo_2025_q2";TABLE="TD_IHH_INTMOVIL_ITE_VA.csv";TEXT="#3c3c3b"
def load_raw(path):
...
def _plot(d,m,out):
    fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.045),.95,.89,boxstyle="round,pad=.01,rounding_size=.018",lw=0,fc="#F8F8FA",transform=fig.transFigure,zorder=-1));fig.text(.045,.9," ",bbox=dict(boxstyle="round,pad=1.5",fc="#4a7d75",ec="none"));fig.text(.061,.9,"Figura C.16.",fontsize=14,fontweight="bold",color=TEXT,va="center");fig.text(.153,.9,f"Herfindahl-Hirschman (IHH). Concentración del mercado de Internet móvil (2013-{m['anio']})",fontsize=14,color=TEXT,va="center");ax=fig.add_axes([.16,.17,.75,.66]);ordered=d.sort_values("anio",ascending=False);bars=ax.barh(np.arange(len(ordered)),ordered.ihh,color="#335a5c",height=.58)
