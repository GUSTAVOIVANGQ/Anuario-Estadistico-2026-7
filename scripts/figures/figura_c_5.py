...
import pandas as pd
FIGURE_ID="C.5"; SOURCE_ID="crt_bit_todo_2025_q2"; TABLE="TD_LINEAS_HIST_TELMOVIL_ITE_VA.csv"; TEXT="#3c3c3b"; CREAM="#F8F8FA"
...
def _frame(title,root):
    fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.045),.95,.89,boxstyle="round,pad=.01,rounding_size=.018",lw=0,fc=CREAM,transform=fig.transFigure,zorder=-1));fig.text(.045,.90," ",bbox=dict(boxstyle="round,pad=1.5",fc="#4a7d75",ec="none"),zorder=20);fig.text(.061,.90,"Figura C.5.",fontsize=14,fontweight="bold",color=TEXT,va="center",zorder=21);fig.text(.145,.90,title,fontsize=14,color=TEXT,va="center",zorder=21);return fig
def _plot(d,m,out,root):
    ...
    series=[d.L_PREPAGO_E,d.L_POSPAGO_E,d.L_POSPAGOC_E,d.L_POSPAGOL_E,d.L_NO_ESPECIFICADO_E];labels=["Prepago","Pospago","Pospago controlado","Pospago libre","Sin segmento especificado"];colors=["#86adae","#64a0a1","#5c9596","#4c7d7e","#3b6667"]
    ax.stackplot(x,*series,labels=labels,colors=colors,alpha=.96);ax.plot(x,d.L_TOTAL_E,color="#132b2d",lw=2.2,marker="o",ms=3,label="Líneas totales")
    for r in d.itertuples(index=False):ax.annotate(f"{r.L_TOTAL_E:.1f}",(r.anio,r.L_TOTAL_E),xytext=(0,6),textcoords="offset points",ha="center",fontsize=6.5,fontweight="bold",color=TEXT,bbox=dict(boxstyle="round,pad=.3,rounding_size=.8",fc="white",ec="#132b2d",lw=.8))
