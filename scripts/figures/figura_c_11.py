...
import pandas as pd
FIGURE_ID="C.11";SOURCE_ID="crt_bit_todo_2025_q2";TABLE="TD_LINEAS_HIST_INTMOVIL_ITE_VA.csv";TEXT="#4B4B83";COLS=["L_PREPAGO_E","L_POSPAGO_E","L_POSPAGOC_E","L_POSPAGOL_E","L_NO_ESPECIFICADO_E","L_TOTAL_E"]
def load_raw(path):
...
def _plot(d,m,out):
    fig=plt.figure(figsize=(16,9),facecolor="white");fig.add_artist(patches.FancyBboxPatch((.025,.045),.95,.89,boxstyle="round,pad=.01,rounding_size=.018",lw=0,fc="#FBFBF7",transform=fig.transFigure,zorder=-1));fig.text(.045,.9," ",bbox=dict(boxstyle="round,pad=1.5",fc="#F58F82",ec="none"),zorder=20);fig.text(.061,.9,"Figura C.11.",fontsize=14,fontweight="bold",color=TEXT,va="center",zorder=21);fig.text(.153,.9,f"Líneas del servicio móvil de acceso a Internet (2010-{m['anio']}) [millones]",fontsize=14,color=TEXT,va="center",zorder=21)
    ax=fig.add_axes([.07,.20,.86,.62]);series=[d.L_PREPAGO_E,d.L_POSPAGO_E,d.L_POSPAGOC_E,d.L_POSPAGOL_E,d.L_NO_ESPECIFICADO_E];ax.stackplot(d.anio,*series,colors=["#ADDCDF","#65BED8","#4BA7C9","#317DA3","#4B4B83"],labels=["Prepago","Pospago","Pospago controlado","Pospago libre","Sin especificar"]);ax.plot(d.anio,d.L_TOTAL_E,color=TEXT,lw=2,marker="o",label="Total")
    for r in d.itertuples(index=False):
        ax.annotate(f"{r.L_TOTAL_E:.1f}",(r.anio,r.L_TOTAL_E),xytext=(0,6),textcoords="offset points",ha="center",fontsize=7,fontweight="bold",color=TEXT)
