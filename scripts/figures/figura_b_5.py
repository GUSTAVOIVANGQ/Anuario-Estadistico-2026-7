"""B.5: script autónomo de adquisición, cálculo, reporte, texto y gráfica."""
...
def _plot_series(figure_id: str, data: pd.DataFrame, meta: dict, output: Path) -> None:
    ...
    bars = ax.bar(x, y, color=TEAL, width=0.70, edgecolor="none", zorder=2)
    for bar, value in zip(bars, y):
        ax.annotate(
            f"{value:,.0f}", (bar.get_x() + bar.get_width() / 2, value),
            xytext=(0, 8), textcoords="offset points", ha="center", va="bottom",
            fontsize=7.2, fontweight="bold", color=TEXT,
            bbox=dict(boxstyle="round,pad=0.25,rounding_size=0.7",
                      facecolor="white", edgecolor=TEAL, linewidth=0.8),
        )
    step = 1 if len(data) <= 30 else 2
    tick_indices = list(range(0, len(data), step))
    if tick_indices[-1] != len(data) - 1:
        tick_indices.append(len(data) - 1)
    ax.set_xticks(
        x[tick_indices], data["ANIO"].astype(str).iloc[tick_indices],
