"""B.16: script autónomo de adquisición, cálculo, reporte, texto y gráfica."""
...
def _plot_technology(data: pd.DataFrame, meta: dict, output: Path) -> None:
    ...
    bg.axis("off")
    colors = ["#132b2d", "#3b6667", "#64a0a1", "#86adae", "#afafaf"]
    ...
    def draw_panel(offset: float, prefix: str, title: str, total: float, growth: float) -> None:
        ...
        pie.legend(
            [patches.Patch(facecolor=color, edgecolor="none") for color in colors],
            data["tecnologia"].tolist(),
            loc="upper center", bbox_to_anchor=(0.5, -0.03), ncol=2,
            frameon=False, fontsize=6.3, labelcolor=TEXT,
            handlelength=1.2, columnspacing=0.8,
        )

        bar = fig.add_axes([0.31 + offset, 0.22, 0.13, 0.29], zorder=3)
        bar.set_title(
            f"Tasa de crecimiento anual\ndic {meta['year'] - 1} - dic {meta['year']}",
            fontsize=8, color=TEXT, pad=7,
        )
