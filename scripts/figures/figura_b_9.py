"""B.9: script autónomo de adquisición, cálculo, reporte, texto y gráfica."""
...
def _market_share(frame: pd.DataFrame, figure_id: str) -> tuple[pd.DataFrame, dict]:
    ...
    order = ["América Móvil", "Grupo Televisa", "Megacable-MCM", "Grupo Salinas", "Axtel", "Telefónica", "Otros"]
...
def _footer(fig: plt.Figure, source: str, note: str = "") -> None:
    fig.text(0.058, 0.145, "Fuente:", fontsize=8, fontweight="bold", color=TEXT, va="top")
    fig.text(0.098, 0.145, textwrap.fill(source, 205), fontsize=8, color=TEXT, va="top")
    if note:
        fig.text(0.058, 0.121, "Nota:", fontsize=8, fontweight="bold", color=TEXT, va="top")
        fig.text(0.089, 0.121, textwrap.fill(note, 210), fontsize=8, color=TEXT, va="top")
...
def _plot_share(figure_id: str, data: pd.DataFrame, meta: dict, output: Path) -> None:
    ...
    bar_width = 0.34
    share_colors = [TEAL, LIGHT, TEXT, SALMON, CORAL, "#49A8D0", "#6BC5E7"]
    ...
    for index, category in enumerate(categories):
        ...
        for year_index, (bar, value, base) in enumerate(zip(bars, values, bottom)):
            bars_by_year[year_index].append(bar)
            if value >= 0.25:
                segments_by_year[year_index].append({
                    "index": index, "value": float(value), "base": float(base),
    ...
    ax.set_ylim(-5, 106)
    x_range = len(data) + 0.16
    y_min, y_max = ax.get_ylim()
    clip_y = (0 - y_min) / (y_max - y_min)
    clip_height = 100 / (y_max - y_min)
    for year_index, year_bars in enumerate(bars_by_year):
        left = (year_index - bar_width / 2 + 0.58) / x_range
        clip = patches.FancyBboxPatch(
            (left, clip_y), bar_width / x_range, clip_height,
    ...
    # Etiquetas tipo llamada. Las participaciones grandes permanecen centradas;
    # las pequeñas alternan costados para conservar la lectura del referente.
    for year_index, segments in enumerate(segments_by_year):
        small = [segment for segment in segments if segment["value"] < 8]
        small.sort(key=lambda segment: segment["center"])
        positions = []
        for segment in small:
            label_y = max(segment["center"], positions[-1] + 4.2 if positions else segment["center"])
            positions.append(label_y)
        if positions and positions[-1] > 102:
            shift = positions[-1] - 102
            positions = [position - shift for position in positions]
        small_positions = {id(segment): label_y for segment, label_y in zip(small, positions)}
        ...
        for segment in segments:
            ...
            if value >= 8:
                ax.annotate(
                    f"{value:.2f}%", xy=(year_index, center), xytext=(0, 0),
                    textcoords="offset points", ha="center", va="center",
                    fontsize=6.7, fontweight="bold", color=TEXT, zorder=6,
                    bbox=dict(boxstyle="round,pad=0.30,rounding_size=0.55",
                              facecolor="white", edgecolor="none", alpha=0.97),
                )
            else:
                side = -1 if segment["index"] % 2 == 0 else 1
                label_x = year_index + side * 0.28
                label_y = small_positions[id(segment)]
                ax.annotate(
                    f"{value:.2f}%",
                    xy=(year_index + side * bar_width / 2, center),
                    xytext=(label_x, label_y), textcoords="data",
                    ha="right" if side < 0 else "left", va="center",
                    fontsize=6.2, fontweight="bold", color=TEXT, zorder=6,
                    bbox=dict(boxstyle="round,pad=0.25,rounding_size=0.5",
                              facecolor="white", edgecolor="none", alpha=0.97),
                    arrowprops=dict(arrowstyle="-", color=segment["color"],
    ...
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.15),
              ncol=len(categories), frameon=False, fontsize=7.5, labelcolor=TEXT,
