...
import numpy as np
import pandas as pd
FIGURE_ID="C.15";SOURCE_ID="crt_bit_todo_2025_q2";TABLE="TD_MARKET_SHARE_INTMOVIL_ITE_VA.csv";TEXT="#3c3c3b"
ORDER=["América Móvil","AT&T","Grupo Walmart","Telefónica","Otros"];COLORS=["#1e6284","#667489","#1b4044","#368491","#728781"]
def load_raw(path):
...
def _plot(d,m,out):
    ...
    fig.text(.045,.9," ",bbox=dict(boxstyle="round,pad=1.5",fc="#4a7d75",ec="none"));fig.text(.061,.9,"Figura C.15.",fontsize=14,fontweight="bold",color=TEXT,va="center");fig.text(.153,.9,f"Participación de mercado del servicio móvil de acceso a Internet (2013-{m['anio']})",fontsize=14,color=TEXT,va="center")
    ax=fig.add_axes([.06,.20,.88,.62]);years=d.anio.to_numpy(float);width=.45;bottom=np.zeros(len(d));segments=[[] for _ in range(len(d))]
    ...
    ax.set_ylim(0,105);ax.set_xticks(years,d.anio.astype(str),fontsize=9,fontweight="bold",color=TEXT);ax.set_yticks([]);ax.tick_params(colors=TEXT,length=0);ax.spines[:].set_visible(False);ax.legend(ncol=5,loc="lower center",bbox_to_anchor=(.5,-.15),frameon=False)
